...
function SectionBlock({ section, idx }: { section: VerticalSection; idx: number }) {
  const bg = idx % 2 === 0 ? "var(--rd-bg-white)" : "var(--rd-bg-subtle)";

  if (section.type === "features") {
    return (
      <section className={sectionStyles.features} style={{ background: bg }}>
        <div className="container mx-auto max-w-6xl px-4">
          <div className="text-center mb-12">
            <h2 className="text-2xl md:text-3xl font-bold mb-4" style={{ color: "var(--rd-text-heading)", fontFamily: "var(--rd-font-heading)" }}>
              {section.heading}
            </h2>
            {section.intro && (
              <p className="text-lg max-w-3xl mx-auto" style={{ color: "var(--rd-text-secondary)" }}>
                {section.intro}
              </p>
            )}
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {section.features.map((f, i) => (
              <div
                key={i}
                className="rounded-[18px] border-[1.5px] p-6 transition-all hover:border-[#93c5fd]"
                style={{ borderColor: "var(--rd-border-light)", background: "var(--rd-bg-white)" }}
              >
                <h3 className="text-lg font-bold mb-2" style={{ color: "var(--rd-text-heading)", fontFamily: "var(--rd-font-heading)" }}>
                  {f.title}
                </h3>
    ...
    return (
      <section className={sectionStyles.bullets} style={{ background: bg, borderColor: "var(--rd-border-light)" }}>
        <div className="container mx-auto max-w-3xl px-4">
          ...
          {section.intro && (
            <p className="text-base mb-6" style={{ color: "var(--rd-text-secondary)" }}>
              {section.intro}
            </p>
          )}
